(* E. Sorokin  2008, Ver 1.7 *)
(* Version for Delphi 4 and above, tested up to Delphi 7 *)
    ...
    Case Typ of
      ...
      scFloat :
        begin
          ...
          if (Result <= 0) then Exit;
          if (FType < 0) then begin     // Scan OK, but no assignment was requested (e.g. NIL)
             asm fstp st(0); end;       // The stack must be cleared!
             Exit;
          end;
          if Negative then asm fchs; end;
          asm fstp [X]; end;
    ...
    Case Typ of
      ...
      scFloat :   begin
                    ...
                    if FType >= 0 then {Check argument conflict}
                      Case Args[Index].VType of
                        vtPointer : {OK};
                        vtExtended : if (SizeType <> scExtended) then goto BadArg;
                        else goto BadArg;
                      end;
