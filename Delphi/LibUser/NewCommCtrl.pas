...
type
  ...
  PNMToolTipsCreated = ^TNMToolTipsCreated;
...
{$IFDEF WHISTLER}
  ...
  ILC_MIRROR              = $2000;      // Mirror the icons contained, if the process is mirrored
  
  ILC_PERITEMMIRROR       = $8000;      // Causes the mirroring code to mirror each item when inserting a set of images, verses the whole strip
{$ENDIF}
...
type
  PHD_TextFilterA = ^THD_TextFilterA;
  PHD_TextFilterW = ^THD_TextFilterW;
  PHD_TextFilter  = ^THD_TextFilterA;
  ...
  THD_TextFilter  = _HD_TEXTFILTERA;
...
function Header_EditFilter(hwnd: HWND; i: integer; fDiscardChanges: LPARAM): integer;
...
type
  ...
  TTBMetrics =
    packed record
      ...
      cxBarPad,      // BARPAD
      cyBarPad,
      cxButtonSpacing,      // BUTTONSPACING
      cyButtonSpacing: integer;
    end;
  
  PTBMetric = ^TTBMetrics;
...
function ListView_SetTileWidth(hwnd: HWND; cpWidth: integer): integer;
...
function ListView_InsertGroup(hwnd: HWND; index: integer; pgrp: PLVGroup): integer;

function ListView_SetGroupInfo(hwnd: HWND; iGroupId: integer; pgrp: PLVGroup): integer;

function ListView_GetGroupInfo(hwnd: HWND; iGroupId: integer; pgrp: PLVGroup): integer;
...
function ListView_SetGroupMetrics(hwnd: HWND; pGroupMetrics: PLVGroupMetrics): integer;

function ListView_GetGroupMetrics(hwnd: HWND; pGroupMetrics: PLVGroupMetrics): integer;

function ListView_EnableGroupView(hwnd: HWND; fEnable: bool): integer;


type
  PFNLVGROUPCOMPARE = function(lParam1, lParam2: integer; lParam3: POINTER): integer stdcall;
  TLVGroupCompare = PFNLVGROUPCOMPARE;

  
  tagLVINSERTGROUPSORTED =
    packed record
      pfnGroupCompare : TLVGroupCompare;
      pvData : POINTER;
      lvGroup : TLVGroup;
    end;
...
function ListView_SortGroups(hwnd: HWND; pfnGroupCompare: TLVGroupCompare;
  plv: PLVInsertGroupSorted): integer;

function ListView_InsertGroupSorted(hwnd: HWND; structInsert: PLVInsertGroupSorted;
  index: integer): integer;

function ListView_RemoveAllGroups(hwnd: HWND): integer;

function ListView_HasGroup(hwnd: HWND; dwGroupId: integer): bool;
...
function ListView_SetTileViewInfo(hwnd: HWND; ptvi: PLVTileViewInfo): integer;

function ListView_GetTileViewInfo(hwnd: HWND; ptvi: PLVTileViewInfo): integer;

function ListView_SetTileInfo(hwnd: HWND; pti: PLVTileInfo): integer;

function ListView_GetTileInfo(hwnd: HWND; pti: PLVTileInfo): integer;
...
function ListView_SetInsertMark(hwnd: HWND; lvim: PLVInsertMark): bool;

function ListView_GetInsertMark(hwnd: HWND; lvim: PLVInsertMark): bool;

function ListView_InsertMarkHitTest(hwnd: HWND; point: TPoint; lvim: PLVInsertMark): integer;

function ListView_GetInsertMarkRect(hwnd: HWND; rc: TRect): integer;
...
function ListView_SetInfoTip(hwndLV: HWND; plvInfoTip: PLVSetInfoTip): bool;
...
function TreeView_SetItemState(hwndTV: HWND; hti: HTreeItem; data: cardinal; _mask: UINT): UINT;
...
function TreeView_GetItemState(hwndTV: HWND; hti: HTreeItem; mask: cardinal): UINT;
...
function TreeView_GetLineColor(hwnd: HWND): TColorRef;
...
function Header_EditFilter(hwnd: HWND; i: integer; fDiscardChanges: LPARAM): integer;
begin
  Result := SendMessage(hwnd,HDM_EDITFILTER,WPARAM(i),
...
function Header_ClearAllFilters(hwnd: HWND): integer;
begin
  Result := SendMessage(hwnd, HDM_CLEARFILTER, -1, 0);
end;
...
function ListView_SortItemsEx(hwndLV: HWND; pfnCompare: TLVCompare; lPrm: Longint): Bool;
begin
  Result := Bool( SendMessage(hwndLV, LVM_SORTITEMSEX, lPrm,
    Longint(@pfnCompare)) );
end;
...
function ListView_SetTileWidth(hwnd: HWND; cpWidth: integer): integer;
begin
  Result := SendMessage(hwnd, LVM_SETTILEWIDTH, WPARAM(cpWidth), 0);
end;

function ListView_SetView(hwnd: HWND; iView: dword): dword;
begin
  Result := DWORD(SendMessage(hwnd, LVM_SETVIEW, WPARAM(iView), 0));
end;
...
function ListView_EnableGroupView(hwnd: HWND; fEnable: bool): integer;
begin
  Result := SendMessage(hwnd, LVM_ENABLEGROUPVIEW, integer(fEnable), 0);
end;

function ListView_SortGroups(hwnd: HWND; pfnGroupCompare: TLVGroupCompare;
  plv: PLVInsertGroupSorted): integer;
begin
  Result := SendMessage(hwnd, LVM_SORTGROUPS, integer(@pfnGroupCompare), LPARAM(plv));
end;
...
function ListView_HasGroup(hwnd: HWND; dwGroupId: integer): bool;
begin
  Result := BOOL(SendMessage(hwnd, LVM_HASGROUP, dwGroupId, 0));
end;
...
function ListView_MapIndexToID(hwnd: HWND; index: UINT): UINT;
begin
  Result := UINT(SendMessage(hwnd, LVM_MAPINDEXTOID, index, 0));
end;

function ListView_MapIDToIndex(hwnd: HWND; id: UINT): UINT;
begin
  Result := UINT(SendMessage(hwnd, LVM_MAPIDTOINDEX, id, 0));
end;
{$ENDIF}
...
function TreeView_SetCheckState(hwndTV: HWND; hti: HTreeItem; fCheck: bool): UINT;
begin
  Result := TreeView_SetItemState(hwndTV,hti,
    INDEXTOSTATEIMAGEMASK(integer(fCheck)),TVIS_STATEIMAGEMASK);
end;

function TreeView_GetItemState(hwndTV: HWND; hti: HTreeItem; mask: cardinal): UINT;
begin
  Result := UINT(SendMessage(hwndTV, TVM_GETITEMSTATE, integer(hti), mask));
end;

function TreeView_GetCheckState(hwndTV: HWND; hti: HTreeItem): UINT;
begin
  Result := (UINT(SendMessage(hwndTV,TVM_GETITEMSTATE,integer(hti),TVIS_STATEIMAGEMASK) shr 12) -1);
end;
...
function TreeView_MapHTREEITEMToAccID(hwnd: HWND; hti: HTreeItem): UINT;
begin
  Result := UINT(SendMessage(hwnd, TVM_MAPHTREEITEMTOACCID, integer(hti), 0));
end;
{$ENDIF}
