...
uses
  ...
  {$IFDEF DFS_COMPILER_4_UP} ImgList, {$ENDIF} Graphics, StdCtrls, Menus, Dialogs,
  Common2;
...
type
  ...
  published
    ...
    property Root: HKEY read FRootKey write FRootKey default HKEY_CURRENT_USER;
...
{$ENDIF}
    ...
    procedure DoHeaderMouseDown(var Message: TWMMouse; Button: TMouseButton;
      Shift: TShiftState);
    procedure DoHeaderMouseUp(var Message: TWMMouse; Button: TMouseButton);
  protected
...
{$ENDIF}
    ...
    function CustomSortProc(Item1, Item2: TListItem;
        var CompResult: integer): Boolean; virtual;
...
{$ENDIF}
    ...
    procedure HeaderMouseDown(Button: TMouseButton; Shift: TShiftState;
      X, Y: Integer); dynamic;
    procedure HeaderMouseUp(Button: TMouseButton; Shift: TShiftState;
      X, Y: Integer); dynamic;
...
{$ENDIF}
    property OnHeaderMouseDown: TMouseEvent read FOnHeaderMouseDown write FOnHeaderMouseDown;
    property OnHeaderMouseUp: TMouseEvent read FOnHeaderMouseUp write FOnHeaderMouseUp;
  public
...
begin
  ...
  try
    ...
    if not TCustomEnhListView(Data).CustomSortProc(Item1, Item2, Result) then
      Exit;
...
begin
  ...
  try
    ...
    Reg.OpenKey(FRegistryKey + '\Columns', True);
    Reg.WriteString( 'Sizes', s);
  finally
    Reg.Free;
  end;
end;
...
begin
  ...
  try
    Reg.RootKey := FRootKey;
    Reg.OpenKey(FRegistryKey + '\Sort', True);
    Reg.WriteBool( 'Ascending', Ascending);
    Reg.WriteInteger( 'SortCol', SortCol);
  finally
    Reg.Free;
  end;
end;
...
begin
  ...
  try
    Reg.RootKey := FRootKey;
    Reg.OpenKey(FRegistryKey + '\Sort', True);
    Ascending := Reg.ReadBoolDef( 'Ascending', TRUE);
    SortCol := Reg.ReadIntegerDef( 'SortCol', 0);
  finally
    Reg.Free;
  end;
end;
...
begin
  ...
  try
    Reg.RootKey := FRootKey;
    Reg.OpenKey(FRegistryKey + '\ViewStyle', True);
    Reg.WriteInteger( 'ViewStyle', STYLE_VAL[Style]);
  finally
    Reg.Free;
  end;
end;
...
begin
  ...
  try
    Reg.RootKey := FRootKey;
    Reg.OpenKey(FRegistryKey + '\ViewStyle', True);
    i := Reg.ReadIntegerDef( 'ViewStyle', -1);
...
{$ENDIF}
  {FSortUpBmp.Free;
  FSortDownBmp.Free;}
  FreeAndNil(FSortUpBmp);
  FreeAndNil(FSortDownBmp);
  {FCanvas.Free}
  FreeAndNil(FCanvas);
...
begin
  ...
  begin
    ...
    if (not assigned(FOnDrawHeader)) and (not FOwnerDrawHeader {FShowSortArrows}) then
    begin
      {FCanvas.Free;
      FCanvas := NIL;}
      FreeAndNil(FCanvas);
...
{$ENDIF}
  ...
  begin
    if (FLastColumnClicked = -1) and (Columns.Count > 0) then
...
begin
  //SetShowSortArrows(FShowSortArrows);
  FSortDirty := FALSE;
...
begin
  ...
  SetColumnsOwnerDrawFlag(assigned(FOnDrawHeader) or FOwnerDrawHeader {FShowSortArrows});
end;
...
begin
  // TODO: fix - possible access violation on vista (mainly on fast user switch)
...
begin
  ...
  FOwnerDrawHeader := ShowSortArrows and not FComCtlVersion6;
  {FSortUpBmp.Free;
  FSortDownBmp.Free;}
  FreeAndNil(FSortUpBmp); // IMPORTANT: fixes access denied-exceptions!
  FreeAndNil(FSortDownBmp);
  if FOwnerDrawHeader then begin
    ...
    for x := 0 to Columns.Count - 1 do begin
      ...
      begin
        if ShowSortArrows and (LastColumnClicked = x) and
          ((AutoColumnSort <> acsNoSort) or (assigned(OnSortItems))) then
          Item.Fmt := Item.Fmt or Flag
        else
          Item.Fmt := Item.Fmt and not Flag;
...
begin
  ...
  begin
    ...
    if Header_GetItem(HeaderHandle, x, Item) then
    begin
      Item.fmt := Item.fmt and not HDF_SORTUP and not HDF_SORTDOWN;
      if OwnerDrawn then
        Item.Fmt := Item.Fmt or HDF_OWNERDRAW
      else begin
        Item.Fmt := Item.Fmt and not HDF_OWNERDRAW;
        if FShowSortArrows and (FLastColumnClicked = x) then
          Item.fmt := Item.fmt or Flag;
      end;
      Header_SetItem(HeaderHandle, x, Item);
    end;
...
begin
  ...
  begin
    ...
    if not FOwnerDrawHeader then
      SetShowSortArrows(FShowSortArrows)
    else if FShowSortArrows then
...
//            if NoColumnResize then
        ...
        WM_LBUTTONDOWN: DoHeaderMouseDown(TWMMouse(Message), mbLeft, []);
        WM_LBUTTONDBLCLK: DoHeaderMouseDown(TWMMouse(Message), mbLeft, [ssDouble]);
        WM_RBUTTONDOWN: DoHeaderMouseDown(TWMMouse(Message), mbRight, []);
        WM_RBUTTONDBLCLK: DoHeaderMouseDown(TWMMouse(Message), mbRight, [ssDouble]);
        WM_MBUTTONDOWN: DoHeaderMouseDown(TWMMouse(Message), mbMiddle, []);
        WM_MBUTTONDBLCLK: DoHeaderMouseDown(TWMMouse(Message), mbMiddle, [ssDouble]);
        WM_LBUTTONUP: DoHeaderMouseUp(TWMMouse(Message), mbLeft);
        WM_RBUTTONUP: DoHeaderMouseUp(TWMMouse(Message), mbRight);
        WM_MBUTTONUP: DoHeaderMouseUp(TWMMouse(Message), mbMiddle);
        WM_NCDESTROY:
          begin
...
procedure TCustomEnhListView.HeaderMouseDown(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if Assigned(FOnHeaderMouseDown) then FOnHeaderMouseDown(Self, Button, Shift, X, Y);
end;

procedure TCustomEnhListView.DoHeaderMouseDown(var Message: TWMMouse; Button: TMouseButton;
  Shift: TShiftState);
var
  Pt: TPoint;
begin
  with Message do begin
    ...
    Windows.ClientToScreen(FHeaderHandle, Pt);
    HeaderMouseDown(Button, KeysToShiftState(Keys) + Shift, Pt.X, Pt.Y);
  end;
end;

procedure TCustomEnhListView.HeaderMouseUp(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if Assigned(FOnHeaderMouseUp) then FOnHeaderMouseUp(Self, Button, Shift, X, Y);
end;

procedure TCustomEnhListView.DoHeaderMouseUp(var Message: TWMMouse; Button: TMouseButton);
var
  Pt: TPoint;
begin
  with Message do begin
    Pt := SmallPointToPoint(Pos);
    Windows.ClientToScreen(FHeaderHandle, Pt);
    HeaderMouseUp(Button, KeysToShiftState(Keys), Pt.X, Pt.Y);
  end;
end;

function TCustomEnhListView.CheckComCtlVersion(MajorHi, MajorLo,
   MinorHi, MinorLo: word): boolean;
begin
  Result := CheckDLLVersion('COMCTL32.DLL', MajorHi, MajorLo, MinorHi, MinorLo);
end;

function TCustomEnhListView.CustomSortProc(Item1, Item2: TListItem;
