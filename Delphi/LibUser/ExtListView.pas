...
{$IFDEF DFS_COMPILER_4_UP}
  ImgList,
{$ENDIF}
  StdCtrls, Menus, EnhListView, Dialogs;
...
type
  ...
  TLVVMChangedEvent = procedure(Sender: TObject; ItemIndex: Integer; Change: Integer)
      of object;
...
{$ENDIF}
    ...
    property OnVMChanged: TLVVMChangedEvent
      read FOnVMChange
      write FOnVMChange;
...
type
  ...
  TDummyItem = class(TListItem)
  private
    FIndex: Integer;
  public
    property Index read FIndex write FIndex;
  end;
...
begin
  ...
  try
    Reg.RootKey := Root;
    Reg.OpenKey(RegistryKey + '\Columns', True);
    Reg.WriteString( 'Order', s);
  finally
    Reg.Free;
  end;
end;
        ...
        begin
          with PNMListView(Pointer(Message.NMHdr))^ do begin
            if FVirtualMode then begin
              if Assigned(FOnVMChange) then
                FOnVMChange(Self, iItem, uChanged);
            end
            else
              inherited;
...
begin
  ...
  SetColumnsOwnerDrawFlag(assigned(OnDrawHeader) or FOwnerDrawHeader {ShowSortArrows});
end;
...
var
  i: Integer;
begin
  if not (assigned(OnDrawHeader) or FOwnerDrawHeader {ShowSortArrows} ) then
    if HandleAllocated then
...
begin { UpdateColumnImage }
  if assigned(OnDrawHeader) or FOwnerDrawHeader {ShowSortArrows} then
    exit;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (csLoading in ComponentState) and (UpdateCount < 1) then begin
        Items.Clear;
