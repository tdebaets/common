...
type
  ...
  THashTableIterator = function(Table: THashTable; Key: String;
  ...
  THashTable = class(TAbstractContainer)
    ...
    public
      ...
      procedure Insert(const aKey : string; aData : pointer); overload;
      ...
      function Search(const aKey : string; var aData : pointer) : boolean; overload;

      procedure Insert(const aKey : string; aData : Integer); overload;
      function Search(const aKey : string; var aData : Integer) : boolean; overload;
...
{--------}
function THashTable.Iterate2(Action: THashTableIterator; Backwards: Boolean;
  ExtraData: Pointer): Pointer;
var
  Inx : integer;
begin
  ...
  if Backwards then begin
    for Inx := pred(htlTableSize) downto 0 do
      with PHashElementArray(htlArray)^[Inx] do begin
        if (heState = hesInUse) then
          if not Action(Self, heString, heData, ExtraData) then begin
            Result := heData;
            Exit;
          end;
      end;
...
procedure THashTable.Insert(const aKey : string; aData : Integer);
begin
  Insert(aKey, Pointer(aData));
end;

function THashTable.Search(const aKey : string; var aData : Integer) : boolean;
begin
  Result := Search(aKey, Pointer(aData));
end;
{====================================================================}
