...
const
  UnitVersioning: TUnitVersionInfo = (
    RCSfile: '$URL: https://jcl.svn.sourceforge.net:443/svnroot/jcl/tags/JCL-1.105-Build3400/jcl/source/windows/JclWideFormat.pas $';
...
begin
  Result := EConvertError.CreateFmt(RsFormatNoArgument, [ArgIndex]);
end;

function FormatNoArgumentErrorEx(const Format: WideString; const FormatStart, FormatEnd, ArgIndex: Cardinal): Exception;
begin
  Result := EConvertError.CreateFmt(RsFormatNoArgumentEx, [ArgIndex, Copy(Format, FormatStart, FormatStart - FormatEnd + 1)]);
end;

function FormatSyntaxError(const CharIndex: Cardinal): Exception;
begin
  Result := EConvertError.CreateFmt(RsFormatSyntaxError, [CharIndex]);
end;
...
begin
  ...
  Result := EConvertError.CreateFmt(RsFormatBadArgumentType, [FoundType, ArgIndex, AllowedTypes]);
end;
...
begin
  FoundType := VarRecTypes[VType];
  AllowedTypes := GetTypeList(Allowed);
  Result := EConvertError.CreateFmt(RsFormatBadArgumentTypeEx, [FoundType, ArgIndex, Copy(Format, FormatStart, FormatEnd - FormatStart + 1), AllowedTypes]);
end;
