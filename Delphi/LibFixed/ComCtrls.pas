...
type
  ...
  private
    ...
    FWidth: TWidth;
    procedure DoChange;
    function GetWidth: TWidth; // LIBFIXED_MODIFIED
  ...
  protected
    ...
    function GetDisplayName: string; override;
  public
    constructor Create(Collection: TCollection); override; // LIBFIXED_MODIFIED
    destructor Destroy; override;
    procedure Assign(Source: TPersistent); override;
...
{ TCustomListView }
  ...
  private
    ...
    procedure DoDragOver(Source: TDragObject; X, Y: Integer; CanDrop: Boolean);
    procedure EditWndProc(var Message: TMessage); // LIBFIXED_MODIFIED
    ...
    procedure WMWindowPosChanged(var Message: TWMWindowPosChanged); message WM_WINDOWPOSCHANGED;
    procedure CNDrawItem(var Message: TWMDrawItem); message CN_DRAWITEM;
    procedure WMCtlColorEdit(var Message: TWMCtlColorEdit); message WM_CTLCOLOREDIT; // LIBFIXED_ADDED
  protected
    ...
    function IsCustomDrawn(Target: TCustomDrawTarget; Stage: TCustomDrawStage): Boolean;
    procedure Notification(AComponent: TComponent; Operation: TOperation); override;
    procedure UpdateColumn(Index: Integer); // LIBFIXED_MODIFIED
...
var
  ...
  FUpdatingColumnOrder: Boolean; { This needs to be moved into TCustomListView }
...
begin
  ...
  begin
    ...
    // Fix for access violation in kernel32: only include LVCF_IMAGE if an image index is set
    // this bug occurred in a TDialogForm (DetailMessageBoxFrm) on Vista and Win7
    // also see TCustomListView.UpdateColumn
...
begin
  // BEGIN_LIBFIXED_MODIFIED
  // Fix for Width returning -1 or -2 instead of the actual width, in
  // contradiction with the docs
  ListView := TListColumns(Collection).Owner;
  if ListView.HandleAllocated then
  begin
    Result := ListView_GetColumnWidth(ListView.Handle, Index);
  ...
  begin
    ...
    try
      for I := 0 to FListColumns.Count - 1 do
        FListColumns.FindItemID(ColumnOrder[I]).Index := I;
    finally
    ...
    begin
      case Msg of
        ...
        CN_SYSCHAR:
          begin
            // BEGIN_LIBFIXED_MODIFIED
            // added: prevent Enter or Escape during label edit from closing the form
            if TWMChar(Message).CharCode in [VK_RETURN, VK_ESCAPE] then
              Result := 0
            else
              WndProc(Message);
            //WndProc(Message);
...
// BEGIN_LIBFIXED_ADDED
procedure TCustomListView.WMCtlColorEdit(var Message: TWMCtlColorEdit);
begin
