...
begin
  ...
  begin
    ...
    begin
      ValidContainer := Parent;
      while ValidContainer <> nil do
      begin
        if ValidContainer.HandleAllocated then Break;
        ValidContainer := ValidContainer.Parent;
      end;
    end
    else
      ValidContainer := Self;

    if ValidContainer <> nil then
    begin
      if NewWidth - FWidth <> 0 then
        Inc(FLastResize.X, NewWidth - FWidth)
      else if HandleAllocated and (FLastClientWidth > 0) and (FLastClientWidth <> ClientWidth) then
      ...
      else if HandleAllocated and (FLastClientHeight > 0) and (FLastClientHeight <> ClientHeight) then
...
begin
  if (DropCtl = nil) or (DropCtl.DockOrientation = doNoOrient) then
  begin
